...
class StorageNodeService(project2_pb2_grpc.StorageNodeServiceServicer):
    ...
    # runs local top -k cosine similartiy search ove the records stored on that node
    def SearchLocal(
        ...
        with self.cv:
            ...
            #return those hits in a SearchLocalResponse -> get hits 
            hits = local_top_k(self.records,query_embedding, request.top_k)
    ...
    def SplitPartition(
        ...
        # TODO:
            ...
            temp_records_list = list(self.records)
            # 2. Split them into two partitions with:
            #       keep_records, move_records, keep_centroid, move_centroid = kmeans_split(local_records)
            keep_records, move_records, keep_centroid, move_centroid = kmeans_split(temp_records_list)
            ...
            # 6. return a SplitPartitionResponse
