...
class ControllerService(project2_pb2_grpc.ControllerServiceServicer):
    ...
    def Put(self, request: PutRequest, context: grpc.ServicerContext) -> PutResponse:
        ...
        with grpc.insecure_channel(target) as channel:
            ...
            split_trigger = False
            if(response.count > MAX_VECTORS_PER_NODE and self.repartitioning == False):
                self.repartitioning = True
                choice_node_num = self.next_node_num
                self.next_node_num += 1 
                threading.Thread(target=self._run_split, args=(response.target, choice_node_num)).start()
                split_trigger = True
            return PutResponse(
                ok=response.ok,
                target=response.target,
                target_count=response.count,
                split_triggered=split_trigger,
            )
